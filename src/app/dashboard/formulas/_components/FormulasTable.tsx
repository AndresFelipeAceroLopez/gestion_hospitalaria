      ...
      <table className="w-full text-sm">
        <thead className="bg-gray-50 border-b border-gray-200">
          <tr>
            ...
            <th className="text-left px-4 py-3 font-medium text-gray-600">Fecha Fórmula</th>
        ...
        <tbody className="divide-y divide-gray-100">
          {formulas.map((formula, index) => (
            <tr key={`f-${formula.formulaId ?? index}`} className="hover:bg-gray-50 transition-colors">
              <td className="px-4 py-3 font-medium text-gray-900">{formula.pacienteNombre}</td>
              <td className="px-4 py-3 text-gray-600">
                {formula.visitaFecha ? formatDate(formula.visitaFecha) : "—"}
                {formula.visitaHora ? ` ${formula.visitaHora}` : ""}
              </td>
              ...
              <td className="px-4 py-3 text-right">
                <form action={deleteFormulaAction.bind}>
                  ...
                  <button
                    ...
                    onClick={(e) => {
                      if (!confirm(`¿Eliminar fórmula de ${formula.pacienteNombre}?`)) e.preventDefault();
                    }}
                  >
                    Eliminar
                  </button>
