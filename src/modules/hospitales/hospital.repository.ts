/**
 * @file src/modules/hospitals/hospital.repository.ts
...
/**
 * Implementación del repositorio para la tabla "hospitales".
  ...
  IPaginableRepository<Hospital, HospitalFilters>
{
     ...
     * @returns Array de hospitales, vacío si no hay resultados
     */
    async findAll(filters?: HospitalFilters): Promise<Hospital[]> {
      ...
      if (filters?.nombre) {
        // like = LIKE insensible a mayusculas/minusculas
        // % al inicio y fin = busqueda "contiene"
        query = query.like("nombre", `%${filters.nombre}%`);
      }
